...
export function OperatorBadge({
  ...
  return (
    <div className={cn('inline-flex items-center', config.gap, className)}>
      ...
      <div
        className={cn(
          'relative flex items-center justify-center rounded-full bg-gradient-to-br from-cyan-500 to-purple-600 text-white font-semibold overflow-hidden',
          config.avatar
        )}
      >
        {avatarUrl ? (
...
export function OperatorInitials({ firstName, lastName }: { firstName: string; lastName: string }) {
  ...
  return (
    <span className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-gradient-to-br from-cyan-500 to-purple-600 text-white text-xs font-bold">
      {initials}
    </span>
  );
}
