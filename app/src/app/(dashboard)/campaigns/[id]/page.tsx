      ...
      <div className="bg-gradient-to-r from-green-500/10 to-green-500/10 border-b border-green-500/30 px-8 py-6">
        <div className="flex justify-between items-center">
          <div>
            <Link
              href="/campaigns"
              className="text-sm text-slate-400 hover:text-green-500 mb-2 inline-block transition-colors"
            >
              ← Back to Campaigns
            </Link>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8">
        ...
        <div className="grid grid-cols-4 gap-5 mb-8">
          <div className="bg-slate-800/50 border border-slate-700/30 rounded-xl p-5 text-center">
            ...
            <div className="text-3xl font-bold text-green-500 mb-2">{campaign.progress}%</div>
        ...
        {activeTab === 'overview' && (
          <div>
            <h2 className="text-xl font-bold text-green-500 mb-4">Email Sequence</h2>
            <div className="space-y-4">
              {emailSteps.map((step: any) => (
                <div
                  key={step.step}
                  className="bg-slate-800/50 border border-slate-700/30 rounded-xl p-5 hover:border-green-500/50 transition-all"
                >
        ...
        {activeTab === 'leads' && (
          <div>
            <h2 className="text-xl font-bold text-green-500 mb-4">Campaign Leads ({leads.length})</h2>
            <div className="bg-slate-800/50 border border-slate-700/30 rounded-xl overflow-hidden">
              <table className="w-full">
                <thead className="bg-slate-900/80">
                  <tr>
                    <th className="px-5 py-4 text-left text-xs font-semibold text-green-500 uppercase">
                      Lead
                    </th>
                    <th className="px-5 py-4 text-left text-xs font-semibold text-green-500 uppercase">
                      Status
                ...
                <tbody>
                  {leads.map((lead: any) => (
                    <tr key={lead.id} className="border-b border-slate-700/20 hover:bg-green-500/5">
